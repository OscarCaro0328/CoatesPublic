...
log_file="/var/log/switchboard/support-commands.log"
scriptname="$(basename "$0")"
timestamp="$(date -u '+%FT%T%z')"
log_template="{
  \"history\" : []
}"
log_block="$(jq --null-input \
...
function dump_log() {
  # Make sure our template is present in the log_file
  log_template_keys="$(jq 'keys' <<< "$log_template")"
  log_file_keys="$(jq 'keys' "$log_file")"
  if [[ "$log_template_keys" != "$log_file_keys" ]]; then
    jq '.' <<< "$log_template" > "$log_file"
  fi

  newlog="$(jq --argjson run "$log_block" '.history[.history | length] = $run' "$log_file")"
  jq '.' <<< "$newlog" > "$log_file"
}
...
function log() {
  ...
  log_block="$(jq --arg log_line "$log_line" \
    '
      .output += [$log_line]
    ' <<< "$log_block"
  )"
